...
# finding nested NPs with Berkeley neural parser
def extract_nps_benepar_as_markables(doc_name, doc_sentences):
    doc_markables = []
    tokens_so_far = 0
    for sentence in doc_sentences:
        ...
        for sent in doc.sents:
            ...
            for constituent in sent._.constituents:
                mention_true = False
                for lbl in constituent._.labels:
                    if lbl in ['NP','PRP']:#,'SBAR']:
                        mention_true = True
                        break
                if mention_true:
                    span_start = tokens_so_far+constituent.start
                    span_end = tokens_so_far+constituent.end-1
                    is_referring = None
                    MIN = None
                    words = constituent.text.split()
                    m = markable.Markable(doc_name, span_start, span_end, MIN, is_referring, words,is_split_antecedent=False,split_antecedent_members=set())
                    head_index = constituent.root.i
                    if head_index in head2markable:
                        head2markable[head_index].append(m)
                    else:
                        head2markable[head_index] = [m]
            # check if the head of NP coincides with the head of another NP and
            # leave only the one with the longest span!
            #('the head leader who guides many', 42, 47), ('the head leader', 42, 44)
            markable_candidates = []
            for head, values in head2markable.items():
                if len(values)==1:
                    markable_candidates.append(values[0])
                else:
                    #markable_candidates.extend(values) if no filtering based on head match
                    ordered_by_span_len = [(i,v.end-v.start) for i,v in enumerate(values)]
                    ordered_by_span_len.sort(key = lambda x: x[1])
                    longest_value_idx = ordered_by_span_len[-1][0]
                    markable_candidates.append(values[longest_value_idx])
            doc_markables.extend(markable_candidates)
...
def print_mention_extraction_stats(doc_name, mode='spacy', debug_show=False):
    ...
    for doc in doc2sentences:
        ...
        if mode=='spacy':
            ...
            extracted_markables = extract_nps_benepar_as_markables(doc, doc_sentences)
        else:
            raise Exception('Unknown mode (can be either spacy or benepar):', mode)
        tp = 0
        fp = 0
        fn = 0
        gold_mentions = []
...
def main():
    if len(sys.argv)!=3:
        raise Exception('You need to specify path to the data and the mode (benepar or spacy)!')
    input_dir = sys.argv[1] #'CRAC_data_2022/'
    mode = sys.argv[2]
    for doc_name in os.listdir(input_dir):
        print_mention_extraction_stats(input_dir+doc_name, mode)
