    ...
    for ds_file in [evaluation_file]:
        ...
        for batch in iterator(instances, num_epochs=1, shuffle=False):
            ...
            for k in range(batch_size):
                ...
                for i in range(len(predicted_lemmas)):
                    span = b["candidates"]["wordnet"]["candidate_spans"][k][i]
                    print(span)
                    token_id = b['tokens']['tokens'][k][span[0]:span[1]+1]
                    print(token_id)
                    word = candidate_generator.bert_tokenizer.convert_ids_to_tokens([token_id.tolist()][0])
                    predicted_lemma_id = None
                    for pp in predicted_lemmas[i]:
                        if pp.partition('%')[0] in word:
                            predicted_lemma_id = pp
                    print(word)
                    print(raw_ids[i][1])
                    print(predicted_lemmas[i])
                    assert predicted_lemma_id is not None
                """
                spans = b["candidates"]["wordnet"]["candidate_spans"][k]
                print(spans)
                token_id = b['tokens']['tokens'][k][spans[0][0]]
                print(candidate_generator.bert_tokenizer.convert_ids_to_tokens([token_id.item()]))
                exit(0)
